...
fn main() {
    ...
    let process = ps_utils::get_target(target).unwrap_or_else(|err| {
        eprintln!("Error getting target process: {}", err);
        std::process::exit(1);
    }).unwrap_or_else(|| {
    ...
    process.print();
    for child in ps_utils::get_child_processes(process.pid).expect("something goes wrong") {
        child.print();
    }
}
